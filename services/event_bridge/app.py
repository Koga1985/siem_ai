...
# _HERE resolves to the directory containing this file.
# Works both in the container (WORKDIR=/app) and in local dev.
_HERE = os.path.dirname(os.path.abspath(__file__))

# Add common module path (container path; works via Docker volume mount)
sys.path.insert(0, "/repo/services")  # noqa: E402
from common.logging_config import setup_logging, log_audit_event  # noqa: E402

# Add this file's directory so normalizers package is always importable
sys.path.insert(0, _HERE)  # noqa: E402
from normalizers.elastic import map_elastic  # noqa: E402
from normalizers.splunk import map_splunk_hec  # noqa: E402
from normalizers.syslog_parser import parse as parse_syslog  # noqa: E402
...
limiter = Limiter(
    get_remote_address, app=app, default_limits=[RATE_LIMIT], storage_uri="memory://"
)
...
try:
    ...
    logger.error(
        "Failed to create queue directory", extra={"path": QUEUE_DIR, "error": str(e)}
    )
    sys.exit(1)

# Load ECS schema — path resolves relative to this file so it works both in
# the container (/repo/schemas/ecs_min.json) and during local development.
_DEFAULT_SCHEMA = os.path.join(_HERE, "../../schemas/ecs_min.json")
_SCHEMA_PATH = os.getenv("ECS_SCHEMA_PATH", _DEFAULT_SCHEMA)
try:
    with open(_SCHEMA_PATH, "r") as f:
        ECS_MIN = json.load(f)
    logger.info("ECS schema loaded successfully")
except Exception as e:
    # Non-fatal: continue without strict schema validation.
    logger.warning(
        "ECS schema unavailable — validation disabled",
        extra={"path": _SCHEMA_PATH, "error": str(e)},
    )
    ECS_MIN = {}
...
def enqueue(evt: dict) -> str:
    ...
    try:
        eid = evt.get("event", {}).get("id") or str(
            abs(hash(json.dumps(evt, sort_keys=True)))
        )
...
def _wrap_generic(evt: dict) -> dict:
    ...
    return {
        "@timestamp": (
            evt.get("@timestamp") or evt.get("time") or datetime.utcnow().isoformat()
        ),
        "event": {
        ...
        "alert": {
            "severity": int(
                evt.get("severity", evt.get("alert", {}).get("severity", 5))
            ),
            "rule": evt.get("rule", evt.get("alert", {}).get("rule", "unknown")),
            "risk_score": float(
                evt.get("risk", evt.get("alert", {}).get("risk_score", 50))
            ),
            "techniques": evt.get(
                "techniques", evt.get("alert", {}).get("techniques", [])
            ),
        },
        "host": {
...
def webhook():  # noqa: C901
    ...
    try:
        ...
        was_wrapped = (
            "raw" in normalised
        )  # _wrap_generic adds 'raw'; native ECS does not
        eid = enqueue(normalised)
        log_audit_event(
            logger, "webhook_received", event_id=eid, source_ip=request.remote_addr
        )
        response = {"status": "queued", "id": eid}
        if was_wrapped:
            response["note"] = "wrapped"
        return jsonify(response), 202

    except Exception as e:
        logger.error(
            "Webhook processing failed",
            extra={"error": str(e), "ip": request.remote_addr},
            exc_info=True,
        )
...
def ingest_splunk():
    ...
    try:
        ...
        eid = enqueue(normalised)
        log_audit_event(
            logger, "splunk_hec_received", event_id=eid, source_ip=request.remote_addr
        )
        return jsonify({"text": "Success", "code": 0, "id": eid}), 200

    except Exception as e:
        logger.error(
            "Splunk HEC processing failed", extra={"error": str(e)}, exc_info=True
        )
...
def ingest_elastic():
    ...
    try:
        ...
        eid = enqueue(normalised)
        log_audit_event(
            logger, "elastic_received", event_id=eid, source_ip=request.remote_addr
        )
...
def ingest_cef():
    ...
    try:
        ...
        eid = enqueue(normalised)
        log_audit_event(
            logger, "cef_received", event_id=eid, source_ip=request.remote_addr
        )
        return jsonify({"status": "queued", "id": eid}), 202
...
def _handle_syslog_message(raw: str, source_ip: str):
    ...
    try:
        ...
        logger.error(
            "Syslog processing error", extra={"error": str(e), "source": source_ip}
        )
...
def start_syslog_udp(port: int):
    ...
    try:
        ...
        logger.error(
            "Syslog UDP listener failed to start", extra={"port": port, "error": str(e)}
        )
        sys.exit(1)
...
def start_syslog_tcp(port: int):
    ...
    try:
        ...
        logger.error(
            "Syslog TCP listener failed to start", extra={"port": port, "error": str(e)}
        )
...
def _handle_tcp_connection(conn: socket.socket, source_ip: str):
    ...
    try:
        ...
        logger.error(
            "TCP connection error", extra={"error": str(e), "source": source_ip}
        )
    finally:
        conn.close()
...
def _looks_like_splunk_hec(body: dict) -> bool:
    ...
    return (
        ...
        and "event" in body
        and isinstance(body["event"], (dict, str))
    )
...
def ratelimit_handler(e):
    logger.warning(
        "Rate limit exceeded", extra={"ip": request.remote_addr, "path": request.path}
    )

...
from event_bridge import app as event_app  # noqa: E402
...
def valid_ecs_event():
    ...
    return {
        ...
        "event": {"id": "test-event-123", "category": "malware"},
        "alert": {
            "severity": 8,
        ...
        "host": {"hostname": "test-host", "ip": "192.168.1.100"},
...
class TestWebhookEndpoint:
    ...
    def test_webhook_invalid_json(self, client):
        ...
        response = client.post(
            "/ingest/webhook", data="invalid json{", content_type="application/json"
        )
    ...
    def test_webhook_with_api_key(self, client, valid_ecs_event, monkeypatch):
        ...
        # Request without API key — API_KEY is read at import time so
        # monkeypatching the env var has no effect here; just verify no crash.
        client.post(
            "/ingest/webhook",
            data=json.dumps(valid_ecs_event),
            content_type="application/json",
